...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="fw-bold" style="color: #2d1b4e;">Dashboard Kadep</h1>
        <p class="text-secondary">Welcome To EVORIA, <?= $_SESSION['nama'] ?? 'Kadep' ?>!</p>
    </div>
</div>
        ...
        <div>
            ...
            <div>
                ...
                <span class="text-secondary d-block mt-2">
                    Selamat datang di dashboard Kepala Departemen. 
                    Anda bertanggung jawab untuk menyetujui acara dari departemen <strong><?= getNamaDepartemen($conn, $id_departemen_kadep) ?></strong>.
                </span>
            </div>
        </div>
    </div>
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <h3 class="fw-bold mb-0" style="color: #2d1b4e;">
                            <?= mysqli_num_rows(mysqli_query($conn, "SELECT * FROM transaksi_acara WHERE id_departemen = '$id_departemen_kadep'")) ?>
                        </h3>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <h3 class="fw-bold mb-0" style="color: #2d1b4e;">
                            <?= mysqli_num_rows(mysqli_query($conn, "SELECT * FROM transaksi_acara WHERE status = 'Pending Kadep' AND id_departemen = '$id_departemen_kadep'")) ?>
                        </h3>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <h3 class="fw-bold mb-0" style="color: #2d1b4e;">
                            <?= mysqli_num_rows(mysqli_query($conn, "SELECT * FROM transaksi_acara WHERE status = 'Disetujui' AND id_departemen = '$id_departemen_kadep'")) ?>
                        </h3>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <h3 class="fw-bold mb-0" style="color: #2d1b4e;">
                            <?= mysqli_num_rows(mysqli_query($conn, "SELECT * FROM transaksi_acara WHERE status = 'Ditolak' AND id_departemen = '$id_departemen_kadep'")) ?>
                        </h3>
                    </div>
                </div>
            </div>
        ...
        <a href="dashboard.php?page=approval" class="text-decoration-none">
            <div class="card menu-card h-100 p-4 shadow-sm border-0 rounded-4">
                <div class="card-body d-flex flex-column">
                    ...
                    <p class="text-secondary mb-4 flex-grow-1">Setujui atau tolak pengajuan acara dari departemen Anda.</p>
                    <div class="d-flex justify-content-between align-items-center mt-auto">
                        <span class="badge bg-danger rounded-pill px-3 py-2">
                            <?= mysqli_num_rows(mysqli_query($conn, "SELECT * FROM transaksi_acara WHERE status = 'Pending Kadep' AND id_departemen = '$id_departemen_kadep'")) ?> Menunggu
                        </span>
    ...
    <div class="card p-4 shadow-sm border-0 rounded-4">
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    <?php 
                    $acara = mysqli_query($conn, "SELECT t.*, m.nama FROM transaksi_acara t 
                                                   LEFT JOIN mahasiswa m ON t.idMhs = m.idMhs 
                                                   WHERE t.id_departemen = '$id_departemen_kadep' 
                                                   ORDER BY t.id_trsAcara DESC LIMIT 5");
                    if (mysqli_num_rows($acara) > 0):
                        while($row = mysqli_fetch_array($acara)): 
                            $status = $row['status'];
                            $badge_class = '';
                            if($status == 'Pending Kadep') $badge_class = 'bg-warning text-dark';
                    ...
                    <tr>
                        <td colspan="4" class="text-center text-muted py-3">
                            Belum ada acara dari departemen Anda.
                        </td>
                    </tr>
                    <?php endif; ?>
                </tbody>
...
<?php
function getNamaDepartemen($conn, $id_departemen) {
    $q = mysqli_query($conn, "SELECT namaDepartemen FROM departemen WHERE id_departemen = '$id_departemen'");
    $d = mysqli_fetch_assoc($q);
    return $d['namaDepartemen'] ?? 'tidak diketahui';
}
?>